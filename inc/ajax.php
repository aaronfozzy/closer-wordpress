...
function pilau_more_posts_link( $args = null ) {
	...
	$date_query = isset( $r['query']->query_vars['date_query'] ) ? $r['query']->query_vars['date_query'] : null;
	...
	if ( $r['query']->found_posts > $posts_per_page ) {
		...
		<script>
			...
			var pilau_ajax_more_data = {
				...
				's':		        '<?php echo $s; ?>',
				'm':				'<?php echo $r['query']->query['m']; ?>',
				<?php if ( $date_query ) { ?>
				'date_query': {
					<?php foreach ( array( 'after', 'before' ) as $field ) { ?>
						<?php if ( ! empty( $date_query[ $field ] ) ) { ?>
						'<?php echo $field; ?>': {
							'year':        <?php echo $date_query[ $field ]['year'] ?>,
							'month':    <?php echo $date_query[ $field ]['month'] ?>,
							'day':        <?php echo $date_query[ $field ]['day'] ?>
						},
						<?php } ?>
					<?php } ?>
				},
				...
				if ( $meta_queries ) { ?>
				'meta_query':	[
					<?php foreach ( $meta_queries as $meta_query_key => $meta_query ) {
						if ( is_integer( $meta_query_key ) ) {
						?> {
...
function pilau_get_more_posts_ajax() {
	...
	if ( isset( $_REQUEST['date_query'] ) ) {
		$args['date_query'] = $_REQUEST['date_query'];
	}
	...
	// Build output
	closer_content_items( $pilau_loop, 2, false, 'news-opinion', true, false, 'post', true, null, false, false, true );
